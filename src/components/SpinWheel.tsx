...
export const SpinWheel: React.FC<ISpinWheelProps> = ({
  ...
  const spin = () => {
    ...
    if (timerHandle === 0) {
      ...
      frames = 0;
      angleCurrent = Math.random() * Math.PI * 2; // Start at a random angle
      angleDelta = Math.random() * 0.1 + 0.1; // Randomize angle delta
  ...
  const drawSegment = (key: number, lastAngle: number, angle: number) => {
    ...
    while (ctx.measureText(value).width > maxTextWidth) {
      fontSize--;
      ctx.font = `${fontSize}px ${fontFamily}`;
    }

    ctx.fillStyle = contrastColor;
    ctx.fillText(value, size / 2 + 10, 0);
    ctx.restore();
  };
  ...
  return (
    <div id='wheel'>
      ...
      />
      <Modal show={showModal} onClose={() => setShowModal(false)} text={needleText} />
    </div>
  );
};
